package week12.ch12_3.training2;
...
class BlogApiControllerTest {
    ...
    public void findAllArticles() throws Exception {
        ...
        //when
        ResultActions result = mockMvc.perform(get(url).header("Accept", "application/json"));
        
        //then
        result.andExpect(status().isOk());
        result.andExpect(jsonPath("$[0].title").value(title));
        result.andExpect(jsonPath("$[0].content").value(content));        
    }
