...
class Archer extends Character {
    ...
    public void useSkill(Character target) {
        if(this.getArrowNum() < 2) {
            System.out.println(this.getName() + "의 화살 개수가 부족하여 스킬을 사용하지 못했습니다.");
        }
        else {
            target.setHP(target.getHP() - (this.getAP() + 30));
            this.setArrowNum(this.getArrowNum()-2);
            printSkill(target);
        }
    }
